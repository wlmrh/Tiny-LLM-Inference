...
- `benchmark/run_benchmark_comparison.py`: run TinyLLM and/or Transformers benchmark backends, parse their JSON summaries, print a compact comparison table plus readable per-backend detail sections, and optionally emit one final JSON summary with TinyLLM/Transformers ratios.
