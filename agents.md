...
- `benchmark/llama_engine_benchmark.cpp`: run an end-to-end offline LLM benchmark using the real `LLM` path. It supports `--device`, `--warmup`, `--repeat`, `--max-new-tokens`, repeated `--prompt`, and `--json`; it reports load/init time, first-token latency, total latency, prompt/generated token counts, throughput, repeat arrays, and TinyLLM runtime breakdowns (`prepare_inputs_ms`, `prefill_ms`, `decode_ms_total`, `decode_ms_per_token`, `sampling_ms`). Build it via the top-level `benchmark/` CMake directory and run it manually; do not register benchmark runs as regular CTest tests.
...
- `benchmark/run_benchmark_comparison.py`: run TinyLLM and/or Transformers benchmark backends, parse their JSON summaries, print a compact comparison table plus any backend-specific runtime breakdown fields, and optionally emit one JSON summary with TinyLLM/Transformers ratios.
