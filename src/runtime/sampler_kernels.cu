...
__global__ void mark_repetition_history_mask_kernel(const int64_t *history_tokens, const int32_t *history_offsets,
                                                    uint8_t *history_mask, int32_t sample_count, int32_t vocab_size)
{
    ...
    for (int32_t index = begin + static_cast<int32_t>(threadIdx.x); index < end;
...
__global__ void argmax_with_repetition_penalty_kernel(const float *logits, const int32_t *sample_rows,
                                                      const uint8_t *history_mask, const float *penalties,
                                                      int32_t *sampled_tokens, int32_t sample_count, int32_t vocab_size)
{
    const int32_t sample = static_cast<int32_t>(blockIdx.x);
    if (sample >= sample_count)
    {
    ...
    for (int32_t token = static_cast<int32_t>(threadIdx.x); token < vocab_size;
    ...
    for (int32_t stride = kSamplerThreads / 2; stride > 0; stride >>= 1)
    {
        if (threadIdx.x < static_cast<unsigned int>(stride))
        {
            ...
            if (better_argmax_candidate(other_value, other_token, shared_values[threadIdx.x],
                                         ...
                                         cudaStream_t stream)
{
    ...
    mark_repetition_history_mask_kernel<<<sample_count, kSamplerThreads, 0, stream>>>(
        history_tokens, history_offsets, history_mask, sample_count, vocab_size);
                                          ...
                                          int32_t vocab_size, cudaStream_t stream)
{
    ...
    argmax_with_repetition_penalty_kernel<<<sample_count, kSamplerThreads, 0, stream>>>(
        logits, sample_rows, history_mask, penalties, sampled_tokens, sample_count, vocab_size);
    CHECK_CUDA(cudaGetLastError());
}
