...
bool cache_entry_matches(const RopeCosSinCacheEntry &entry, const c10::Device &device, int32_t head_dim,
                         float rope_theta, const std::string &rope_scaling_type, float rope_scaling_factor,
                         float rope_scaling_low_freq_factor, float rope_scaling_high_freq_factor,
                         int32_t rope_scaling_original_max_position_embeddings, int32_t required_positions)
{
    return entry.cos.defined() && entry.sin.defined() && entry.device == device && entry.head_dim == head_dim &&
           entry.rope_theta == rope_theta && entry.rope_scaling_type == rope_scaling_type &&
...
RotaryEmbedding::RotaryEmbedding(int32_t num_attention_heads, int32_t num_key_value_heads, int32_t head_dim,
                                 float rope_theta, std::string rope_scaling_type, float rope_scaling_factor,
                                 float rope_scaling_low_freq_factor, float rope_scaling_high_freq_factor,
                                 int32_t rope_scaling_original_max_position_embeddings, int32_t max_position_embeddings)
    : num_attention_heads_(num_attention_heads), num_key_value_heads_(num_key_value_heads), head_dim_(head_dim),
      rope_theta_(rope_theta), rope_scaling_type_(std::move(rope_scaling_type)),
      rope_scaling_factor_(rope_scaling_factor), rope_scaling_low_freq_factor_(rope_scaling_low_freq_factor),
...
Tensor RotaryEmbedding::inv_freq_for_device(const c10::Device &device) const
{
    ...
    if (rope_scaling_type_ == "llama3")
    {
        ...
        const Tensor smooth_factor = (static_cast<float>(rope_scaling_original_max_position_embeddings_) / wavelen -
        ...
        inv_freq = torch::where(wavelen > low_freq_wavelen, inv_freq / rope_scaling_factor_,
...
{
    const int32_t required_positions =
        rope_cache_length(max_position_embeddings_, rope_scaling_original_max_position_embeddings_);

    {
        std::lock_guard<std::mutex> lock(g_rope_cos_sin_cache_mutex);
        for (const RopeCosSinCacheEntry &entry : g_rope_cos_sin_cache)
        {
            if (cache_entry_matches(entry, device, head_dim_, rope_theta_, rope_scaling_type_, rope_scaling_factor_,
                                    rope_scaling_low_freq_factor_, rope_scaling_high_freq_factor_,
                                    rope_scaling_original_max_position_embeddings_, required_positions))
            {
...
void RotaryEmbedding::forward(const Tensor &positions, Tensor &q, Tensor &k) const
{
    if (q.device().is_cuda())
    {
        ...
        ops::apply_rope(positions, q, k, num_attention_heads_, num_key_value_heads_, head_dim_, cache.first,
                        cache.second);
        return;
    }

    ops::apply_rope(positions, q, k, num_attention_heads_, num_key_value_heads_, head_dim_,
