...
ParsedDType parse_dtype(const std::string &dtype_token)
{
    ...
    throw std::runtime_error("HFSafeTensorLoader::from_file: unsupported dtype token: " + dtype_token + ".");
}
...
size_t tensor_nbytes_from_shape(const std::vector<int64_t> &shape, const ParsedDType &dtype)
{
    ...
    {
        element_count = checked_mul(element_count, static_cast<size_t>(dim), "HFSafeTensorLoader::from_file");
    }
...
HFSafeTensorLoader HFSafeTensorLoader::from_file(const std::string &path)
{
    ...
    const size_t expected_data_start =
        checked_add(kSafeTensorHeaderPrefixBytes, header_length, "HFSafeTensorLoader::from_file");
    ...
    const char *header_begin = reinterpret_cast<const char *>(loader.raw_file_.data() + kSafeTensorHeaderPrefixBytes);
    ...
    for (const auto &item : root_object)
    {
        ...
        {
            throw std::runtime_error("HFSafeTensorLoader::from_file: tensor descriptor pointer must be non-null.");
        }

        const hf_json::Value &tensor_descriptor = *item.second;

        (void)tensor_descriptor.as_object("HFSafeTensorLoader::from_file: tensor descriptor must be object");

        const std::string dtype_token =
            hf_json::require_object_field(tensor_descriptor, "dtype", "HFSafeTensorLoader::from_file")
                .as_string("HFSafeTensorLoader::from_file: dtype must be string");
        const ParsedDType dtype = parse_dtype(dtype_token);

        const std::vector<int64_t> shape =
            parse_shape(hf_json::require_object_field(tensor_descriptor, "shape", "HFSafeTensorLoader::from_file"));

        const auto [offset_begin, offset_end] = parse_offsets(
            hf_json::require_object_field(tensor_descriptor, "data_offsets", "HFSafeTensorLoader::from_file"));

        const size_t expected_bytes = tensor_nbytes_from_shape(shape, dtype);
        const size_t actual_bytes = offset_end - offset_begin;
        if (expected_bytes != actual_bytes)
        {
            throw std::runtime_error("HFSafeTensorLoader::from_file: byte size mismatches shape for key: " +
                                     tensor_key);
        }
    ...
    for (const auto &item : loader.tensor_infos_)
    {
        ...
        const size_t absolute_begin =
            checked_add(loader.data_base_offset_, info.byte_offset, "HFSafeTensorLoader::from_file");
        const size_t absolute_end = checked_add(absolute_begin, info.byte_size, "HFSafeTensorLoader::from_file");

        if (absolute_end > loader.raw_file_.size())
        {
            throw std::runtime_error("HFSafeTensorLoader::from_file: tensor range exceeds file size for key: " + key);
        }
    ...
    for (size_t i = 1; i < spans.size(); ++i)
    {
        ...
        {
            throw std::runtime_error("HFSafeTensorLoader::from_file: overlapping tensor ranges detected: " +
...
Tensor HFSafeTensorLoader::tensor(const std::string &key) const
{
    ...
    const auto options = torch::TensorOptions().dtype(to_torch_scalar_type(info.dtype)).device(c10::kCPU);
    ...
    if (info.storage_dtype == "F32" && (reinterpret_cast<uintptr_t>(data_ptr) % alignof(float)) != 0)
    {
    ...
    return make_tensor_from_blob(const_cast<uint8_t *>(data_ptr), info.shape, info.dtype);
}
