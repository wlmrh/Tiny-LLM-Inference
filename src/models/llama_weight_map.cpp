...
Tensor load_checked_tensor(const WeightMap &weight_map, const std::string &key,
                           const std::vector<int64_t> &expected_shape)
{
    ...
    {
        throw std::runtime_error("load_llama_weights: shape mismatch for key " + key + ", expected=" +
                                 shape_to_string(expected_shape) + ", got=" + shape_to_string(loaded_shape));
    }
...
WeightMap WeightMap::from_safetensors(const HFSafeTensorLoader &loader, const ParallelConfig &parallel_config)
{
...
void WeightMap::add_tensor(const std::string &name, void *data, const std::vector<int64_t> &shape, DType dtype)
{
...
LlamaWeights load_llama_weights(const WeightMap &weight_map, const LlamaConfig &config)
{
    ...
    weights.embed_tokens =
        load_checked_tensor(weight_map, "model.embed_tokens.weight", {config.vocab_size, config.hidden_size});
    ...
    for (int32_t layer_idx = 0; layer_idx < config.num_hidden_layers; ++layer_idx)
    {
        ...
        layer.input_layernorm =
            load_checked_tensor(weight_map, prefix + "input_layernorm.weight", {config.hidden_size});
        layer.q_proj = load_checked_tensor(weight_map, prefix + "self_attn.q_proj.weight",
                                           {config.hidden_size, config.hidden_size});
        layer.k_proj = load_checked_tensor(weight_map, prefix + "self_attn.k_proj.weight",
                                           {kv_hidden_size(config), config.hidden_size});
        layer.v_proj = load_checked_tensor(weight_map, prefix + "self_attn.v_proj.weight",
                                           {kv_hidden_size(config), config.hidden_size});
        layer.o_proj = load_checked_tensor(weight_map, prefix + "self_attn.o_proj.weight",
                                           {config.hidden_size, config.hidden_size});
        layer.post_attention_layernorm =
            load_checked_tensor(weight_map, prefix + "post_attention_layernorm.weight", {config.hidden_size});
        layer.gate_proj = load_checked_tensor(weight_map, prefix + "mlp.gate_proj.weight",
                                              {config.intermediate_size, config.hidden_size});
        layer.up_proj = load_checked_tensor(weight_map, prefix + "mlp.up_proj.weight",
                                            {config.intermediate_size, config.hidden_size});
        layer.down_proj = load_checked_tensor(weight_map, prefix + "mlp.down_proj.weight",
    ...
    weights.lm_head = load_checked_tensor(
        weight_map, weight_map.contains("lm_head.weight") ? "lm_head.weight" : "model.embed_tokens.weight",
