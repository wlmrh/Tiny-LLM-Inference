...
void validate_same_device(std::initializer_list<std::reference_wrapper<const Tensor>> tensors,
                          const char* name)
{
...
{
    return params.metadata != nullptr
        && params.metadata->enabled
        && params.ctx != nullptr
...
{
    if (params.positions == nullptr || params.q == nullptr || params.k == nullptr
        || params.v == nullptr || params.out == nullptr || params.ctx == nullptr)
    {
    ...
    if (k.dim() != 2 || v.dim() != 2 || out.dim() != 2
        ...
        || k.size(1) != kv_hidden_size(params.num_key_value_heads, params.head_dim)
...
void validate_paged_metadata_for_attention(const LlamaAttentionParams& params)
{
    ...
    if (tensor_dtype(seq_indices) != DType::kInt32
        || tensor_dtype(context_lens) != DType::kInt32
        || tensor_dtype(block_tables) != DType::kInt32)
    {
    ...
    if (context_shape.size() != 1 || block_shape.size() != 3
        || block_shape[0] <= params.layer_id || block_shape[1] != context_shape[0])
    {
...
void attention_paged(const Tensor& q, Tensor& out, ExecutionContext& ctx)
{
    ...
    if (q.device().is_cuda() && out.device().is_cuda())
    {
        ...
        cuda::launch_attention_paged_f32(
            q_ptr,
            out_ptr,
            static_cast<int64_t>(tensor_numel(q)),
...
void llama_attention_forward(const LlamaAttentionParams& input_params)
{
    ...
    validate_same_device({
        std::cref(*params.positions),
        std::cref(*params.q),
        std::cref(*params.k),
        std::cref(*params.v),
        std::cref(*params.out)}, "llama_attention");
    validate_paged_metadata_for_attention(params);

    const bool cuda_inputs = any_cuda({
        std::cref(*params.positions),
        std::cref(*params.q),
        std::cref(*params.k),
        std::cref(*params.v),
