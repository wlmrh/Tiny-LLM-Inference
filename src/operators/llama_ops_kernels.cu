...
__global__ void apply_rope_cached_f32_kernel(const int32_t *positions, float *q, float *k, const float *cos_cache,
                                             const float *sin_cache, int64_t rows, int32_t num_attention_heads,
                                             int32_t num_key_value_heads, int32_t head_dim, int64_t cache_rows,
                                             int64_t q_stride, int64_t k_stride)
{
    const int32_t rotary_half = head_dim / 2;
    const int64_t q_items = rows * static_cast<int64_t>(num_attention_heads) * static_cast<int64_t>(rotary_half);
    const int64_t k_items = rows * static_cast<int64_t>(num_key_value_heads) * static_cast<int64_t>(rotary_half);
...
__global__ void silu_multiply_f32_kernel(const float *gate, const float *up, float *out, int64_t numel)
{
    const int64_t idx =
        static_cast<int64_t>(blockIdx.x) * static_cast<int64_t>(blockDim.x) + static_cast<int64_t>(threadIdx.x);
    if (idx >= numel)
                                  ...
                                  int64_t k_stride, cudaStream_t stream)
{
    ...
    const int64_t total = rows * static_cast<int64_t>(num_attention_heads + num_key_value_heads) * rotary_half;
    if (total <= 0)
    {
        return;
    }
    ...
    apply_rope_cached_f32_kernel<<<static_cast<unsigned int>(blocks), kThreadsPerBlock, 0, stream>>>(
        positions, q, k, cos_cache, sin_cache, rows, num_attention_heads, num_key_value_heads, head_dim, cache_rows,
        q_stride, k_stride);
...
void launch_silu_multiply_f32(const float *gate, const float *up, float *out, int64_t numel, cudaStream_t stream)
{
    ...
    silu_multiply_f32_kernel<<<static_cast<unsigned int>(blocks), kThreadsPerBlock, 0, stream>>>(gate, up, out, numel);
    CHECK_CUDA(cudaGetLastError());
}
...
void launch_add_f32(const float *lhs, const float *rhs, float *out, int64_t numel, cudaStream_t stream)
{
    ...
    const int64_t blocks = (numel + kThreadsPerBlock - 1) / kThreadsPerBlock;
    add_f32_kernel<<<static_cast<unsigned int>(blocks), kThreadsPerBlock, 0, stream>>>(lhs, rhs, out, numel);
    CHECK_CUDA(cudaGetLastError());
}
